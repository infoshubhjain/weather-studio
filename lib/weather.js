...
// Open-Meteo updates roughly every 15 minutes, so anything shorter is wasted
// bandwidth. Historical data never changes, so it gets a much longer life.
...
export async function getWeather({ lat, lon, days = 5 }) {
  ...
  // Round the key so near-identical coordinates share a cache entry.
  const k = `${(+lat).toFixed(2)},${(+lon).toFixed(2)}`;
  const [w, air] = await Promise.allSettled([
    cached(`fc:${k}:${days}`, LIVE_TTL, () => getJSON(`${BASE}?${qs}`)),
    cached(`aq:${k}`, LIVE_TTL, () =>
...
export async function getRange({ lat, lon, start, end }) {
  ...
  if (start < today) {
    ...
    calls.push(cached(`ar:${k}:${start}:${to}`, ARCHIVE_TTL, () =>
      getJSON(`${ARCHIVE}?latitude=${lat}&longitude=${lon}&start_date=${start}&end_date=${to}&daily=${fields}&timezone=auto`)));
  }
  if (end >= today) {
    const from = start > today ? start : today;
    calls.push(cached(`fr:${k}:${from}:${end}`, LIVE_TTL, () =>
      getJSON(`${BASE}?latitude=${lat}&longitude=${lon}&start_date=${from}&end_date=${end}&daily=${fields}&timezone=auto`)));
  }
