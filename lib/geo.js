...
export async function geocode(query, { limit = 5 } = {}) {
  ...
  // Both providers start together, but we only *wait* for Nominatim when
  // Open-Meteo came back empty — which in practice means a landmark, since
  // Open-Meteo covers cities and postal codes and Nominatim covers the rest.
  // Waiting for both (the old Promise.allSettled) meant every city search paid
  // for the slower of the two: ~0.5s of pure waiting, every time.
  //
  // The Nominatim promise is deliberately left running when unused. It fills
  // the cache for the next caller, and pre-catching it here is what keeps an
  // abandoned rejection from surfacing as an unhandled rejection.
  const omP = cached(`om:${ck}:${limit}`, GEO_TTL, () =>
  ...
  const osmP = cached(`osm:${ck}:${limit}`, GEO_TTL, () =>
  ...
  const om = await omP;
  const omHits = om.status === 'fulfilled' ? (om.value?.results ?? []) : [];
  const osm = omHits.length ? { status: 'skipped', value: null } : await osmP;

  const out = [];
  if (om.status === 'fulfilled') {
    for (const r of omHits) {
      out.push({
